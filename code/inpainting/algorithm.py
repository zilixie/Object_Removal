...
class Inpainting:
    ...
    def readImage(self, fileName, key):
        ...
        if key not in self._images.keys():
            msg, success = 'Key', False
        else:
            if key == 'source': image = cv.imread(fileName, cv.IMREAD_COLOR)
            if key == 'alpha': image = cv.imread(fileName, 0)
            if image is None: msg = 'Read Image Fail'
            else: 
                self._images[key], success = image, True
    ...
    def writeImage(self, fileName, key):
        ...
        if write: success, msg = True, 'Write Image Succeed'
        else: success, msg = False, 'Write Image Fail'
...
success, errorMessage = exampleBasedInpainting(self)
        ...
        if (alpha is None) or (source is None): return False, 'alpha or source is None.'
        source_shape, alpha_shape = list(source.shape), list(alpha.shape)
        
        if (source_shape[:2] != alpha_shape[:2]): return False, 'unmatched dimension.'
        if (source_shape[2] != 3): return False, 'source Image is not 3-channel uint8 image.'
        if (len(alpha_shape) != 2): return False, 'alpha Image is not 1-channel uint8 image.'
        success = True
